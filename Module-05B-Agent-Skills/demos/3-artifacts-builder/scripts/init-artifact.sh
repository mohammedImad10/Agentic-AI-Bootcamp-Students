...
# A function, not a string: "sed -i ''" stored in a variable word-splits into a
# literal '' argument, which BSD sed reads as a backup SUFFIX and leaves a stray
# index.html'' behind. A function keeps the empty argument empty.
...
# Vite used to scaffold /vite.svg; current templates use /favicon.svg. Match any
# icon link so the tag cannot survive into the Parcel build, where an unresolved
# href is a hard failure.
